    ...
    <Card noborder>
      <div class="md:flex pb-6 items-center justify-between">
        ...
        <div class="md:flex md:space-x-3 items-center flex-none" :class="window.width < 768 ? 'space-x-rb' : ''">
          <router-link :to="`/cih/centers/center/${route.params.zoneId}?name=${route.query.name}`">
      ...
      <div class="-mx-6">
        <vue-good-table :columns="columns" mode="remote" styleClass=" vgt-table  centered " :rows="centers"
          ...
          <template v-slot:table-row="props">
            ...
            <router-link :to="`/cih/zones/center/${props.row.id}?name=${props.row.centerName}`">
  ...
  <Modal title="Confirm action" label="Small modal" labelClass="btn-outline-dark" ref="modalStatus" sizeClass="max-w-md"
    ...
    <template v-slot:footer>
      <div class="flex gap-x-5">
        ...
        <Button :disabled="deleteloading || (!reason && type.toLowerCase() === 'delist')
            " :isLoading="deleteloading" text="Proceed" :btnClass="` btn-sm ${type === 'approve' ? 'btn-success' : 'btn-danger'
            }`" @click="handleStatus" />
      </div>
    </template>
  </Modal>
