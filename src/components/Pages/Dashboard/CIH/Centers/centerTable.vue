<template>
  <div>
    <Card noborder>
      <div class="md:flex pb-6 items-center justify-between">
        ...
        <export-excel :data="members || []" worksheet="members"
          :name="`members.csv`" type="csv">
            ...
            <span v-if="props.column.field == 'action'">
              <Dropdown classMenuItems=" w-[160px]">
                ...
                <template v-slot:menus>
                  ...
                  <div @click="item.doit(props.row, item.name)"
                    :class="`
  ...
  <!-- ///CIH Modal/// -->
  <Modal :title="type" label="Small modal" :labelClass="`${type === 'assign role' ? 'btn-primary-600' : 'btn-outline-danger'
            }`" ref="roleModalChange" sizeClass="max-w-md" :themeClass="`${type === 'assign role' ? 'bg-primary-500' : 'bg-danger-500'
            }`">
    <div>
      <AssignRole v-if="type === 'assign role'" :userId="id" :closeModal="closeRoleModalChange" :isCIH="true"
        :refetch="refetchCenterMembers" />
      <RemoveRole v-if="type === 'remove role'" :userId="id" :closeModal="closeRoleModalChange" :isCIH="true"
        :refetch="refetchCenterMembers" />
    </div>
  </Modal>
</template>
