...
<template>
  <div>
    ...
    <Card noborder>
      <div class="md:flex pb-6 items-center justify-between">
        ...
        <div class="md:flex md:space-x-3 items-center flex-none" :class="window.width < 768 ? 'space-x-rb' : ''">
          <router-link :to="`/cih/zones/zone-members/${route.params.zoneId}?name=${route.query.name}`">
            <Button text="View all members in this zone" btnClass=" btn-dark font-normal btn-sm w-full mt-2 md:mt-0"
              iconClass="text-lg" />
          </router-link>
          <div class="flex justify-between w-full box-border mt-2 md:mt-0">
            ...
            <Dropdown classMenuItems=" min-w-[100px]">
              ...
              <template v-slot:menus>
                <MenuItem>
                <export-excel :data="centers || []" worksheet="centers" :name="`${route.query.name}-centers.csv`"
                  type="csv">
      ...
      <div class="-mx-6">
        <v-pdf ref="pdf" :options="pdfOptions" :filename="`${route.query.name}-members.pdf`">

          <vue-good-table :columns="columns" mode="remote" styleClass=" vgt-table  centered " :rows="centers"
            :isLoading="loading"
            :sort-options="{
            enabled: false,
          }" :pagination-options="{
  ...
  <Modal title="Delete Center" label="Small modal" labelClass="btn-outline-danger" ref="modal" sizeClass="max-w-md"
...
export default {
  ...
  setup() {
    ...
    const loading = computed(() => state.center.getcentersloading);
