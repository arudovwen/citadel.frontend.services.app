    ...
    <Card noborder>
      <div class="md:flex pb-6 items-center justify-between">
        ...
        <div class="md:flex md:space-x-3 items-center flex-none" :class="window.width < 768 ? 'space-x-rb' : ''">
          <VueTailwindDatePicker v-model="query.dateValue" :formatter="formatter" input-classes="form-control h-[36px] min-w-[300px]"
      ...
      <div class="hidden">
        <v-pdf ref="pdf" :options="pdfOptions" :filename="exportFilename">
          <div class="-mx-6">
            <vue-good-table :columns="active === 'inspection'
            ...
            clearSelectionText: 'Clear selection',
              ...
              <template #selected-row-actions>
                <button :disabled="loading" :isLoading="loading" @click="handleReports"
...
export default {
  ...
  setup() {
    ...
    const removeChar = (chars) => {
      return chars?.replace(/<p>/, '').replace(/<\/p>/, '').replace(/="|"/, '').replace(/"$/, '');
    }
    const reports = computed(() => {
      console.log("result = ", removeChar(state?.report?.data[0]?.summaryOfEvent))
      return state?.report?.data.map((item) => {
        item.summaryOfEvent = removeChar(item?.summaryOfEvent)
        return item;
      });
    });
