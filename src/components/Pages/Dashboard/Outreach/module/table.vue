...
<template>
  <div>
    <div className="flex items-center  overflow-x-auto mb-6 border-b border-b-[#DDDDDD]">
    ...
    <Card noborder>
      ...
      <div class="-mx-6">
        <vue-good-table :columns="(currentTab === tabs[0] || currentTab === tabs[3]) ? approvedRequestcolumns : columns"
...
const tabs = ["upcoming outreach", "my outreach", "pending requests", "past outreaches"];
...
watch(currentTab, () => {
  if (currentTab.value === tabs[0] || currentTab.value === tabs[3]) {
    query.status = "approved";
  } else if (currentTab.value === tabs[2]) {
  ...
  query.dateFilter = (currentTab.value === tabs[0] || currentTab.value === tabs[3]) ? "dateOfOutreach" : null;
  query.StartDate = currentTab.value === tabs[0] ? moment(new Date()).format("YYYY-MM-DD") :
    currentTab.value === tabs[3] ? moment(new Date()).subtract("months", 12).format("YYYY-MM-DD") : null;
  query.EndDate = currentTab.value === tabs[3] ? moment(new Date()).subtract("days", 1).format("YYYY-MM-DD") : moment().add("years", 10).format("YYYY-MM-DD");
  query.pageNumber = 1;
