...
<template>
  <div>
    ...
    <Card noborder>
      <div class="md:flex pb-6 items-center justify-between">
        <div
          class="gap-x-4 items-center md:flex"
          :class="width > 768 && 'flex'"
        >
          <InputGroup
          ...
          <VueTailwindDatePicker
            ...
            :formatter="formatter"
            input-classes="form-control w-max-[300px] !w-min-[250px] h-[36px]"
      ...
      <div class="-mx-6">
        <vue-good-table
          :columns="
            currentTab === tabs[0] || currentTab === tabs[3]
  ...
  >
    ...
    <span
      v-if="rejectReasonErr?.length > 0"
      class="mt-2 text-danger-500 text-xs"
    >
...
watch(currentTab, () => {
  ...
  query.dateFilter =
    currentTab.value === tabs[0] || currentTab.value === tabs[3]
      ? "dateOfOutreach"
      : null;
  query.StartDate =
    currentTab.value === tabs[0]
      ...
      ? moment(new Date()).subtract("months", 12).format("YYYY-MM-DD")
      : null;
  query.EndDate =
    currentTab.value === tabs[3]
      ? moment(new Date()).subtract("days", 1).format("YYYY-MM-DD")
      : moment().add("years", 10).format("YYYY-MM-DD");
  query.pageNumber = 1;
