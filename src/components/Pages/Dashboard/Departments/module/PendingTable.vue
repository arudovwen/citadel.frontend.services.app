<template>
      ...
      <div class="">
        <vue-good-table :columns="columns" styleClass="vgt-table" :isLoading="deptloading" :rows="members || []"
          ...
          <template v-slot:table-row="props">
            ...
            <span v-if="props.column.field == 'requestDate'" class="text-slate-500 dark:text-slate-400">
  ...
  <Modal :title="`${type} request`" label="Small modal" :labelClass="type === 'approve' ? 'btn-outline-success' : 'btn-outline-danger'
            " ref="modal" sizeClass="max-w-md" :themeClass="type === 'approve' ? 'bg-success-500' : 'bg-danger-500'">
    ...
    <template v-slot:footer>
      <div class="flex gap-x-5">
        ...
        <Button :isLoading="approveloading" :disabled="approveloading || (!comment && type.toLowerCase() === 'reject')
...
import member from "@/store/app/member";
...
export default {
  ...
  setup() {
    ...
    const departmentName = computed(() => {
      return state.department?.DepartmentName
    })
