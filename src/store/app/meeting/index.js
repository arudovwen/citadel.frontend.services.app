...
export default {
  ...
  mutations: {
    ...
    // removeMeeting
    removeMeeting(state, data) {
      state.meetings = state.meetings.filter((item) => item.id !== data.id);
      toast.error("Meeting Removed", {
        timeout: 2000,
      });
    },
    // updateMeeting
    updateMeeting(state, data) {
      state.meetings.findIndex((item) => {
        if (item.id === data.id) {
          // store data
  ...
  actions: {
    ...
    // removeMeeting
    removeMeeting({ commit }, data) {
      commit("removeMeeting", data);
    },
    // updateMeeting
